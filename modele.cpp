...
int tireDeuxOuQuatre() {
	int a = rand() % 10;
	if (a == 2) { // la probabilité de tirer un 4 étant de 1/10, peu-impporte la valeur tirée par le rand, elle nous servira de repère pour return un quatre car on a une chance sur 10 de trouver la valeur trouvée
		return 4;
	}
	else {
		return 2;
...
Plateau plateauInitial() {
	...
	while (b == d && c == e) {                // boucle permettant de ne pas placer deux fois la même case sur le tableau ( pas avoir les mêmes indices lors du tirage)
		b = (rand() % 4);
		c = (rand() % 4);
	}
...
Plateau Generetkt(Plateau plateau) {
	...
	while (plateau[a][b] != 0) {  // boucle permettant de générer une tuile sur une tuile vide du tableau
		srand(time(NULL));
		a = rand() % 4;
		b = rand() % 4;
	}
...
pair<Plateau, int> deplacementDroite(Plateau plateau) {
	...
	for (int i = 0; i < 4; i++)
		for (int j = 3; j >= 0; j--)
		{
			if (plateau[i][j] == 0) // cas dans lequel la case est vide 
			{
				for (int count = j - 1; count > 0; count--) // ne prend pas en compte la ligne de droite et commence directement a la deuxieme case 
				{
			...
			{
				plateau[i][j] = plateau[i][j] + plateau[i][j - 1]; //on affecte donc à la tuile la valeur des deux adjacentes
				plateau[i][j - 1] = 0; //on vide la tuile dde droite car elle vient de fusionner avec la tuile initiale
				score += plateau[i][j];
			}
		}
	pair<Plateau, int> ok = { plateau, score };
...
pair<Plateau, int> deplacementHaut(Plateau plateau) {
	...
	for (int i = 0; i < 4; i++)
	{
		for (int j = 0; j < 4; j++)
		{
			if (plateau[i][j] == 0)
			{
				for (int count = i + 1; count < 4; count++) // boucle permettant dans un premier point de tout décaller à gauche
					if (plateau[count][j])
					{
						plateau[i][j] = plateau[count][j]; //prend la valeur de la case d'à côté (gauche)
	...
	for (int i = 0; i < 3; i++)
		for (int j = 0; j < 4; j++)
		{
			if (plateau[i][j] == 0) // cas dans lequel la case est vide 
			{
				for (int count = i + 1; count < 3; count++) // ne prend pas en compte la ligne de gauche et commence directement a la  deuxieme case 
				{
					...
					if (plateau[count][j] == plateau[count + 1][j])
					{
						...
						plateau[count + 1][j] = 0; // remet a 0 la case de droite qui vient d'être ajouter a celle a sa gauche
						plateau[count][j] = 0;
						score += plateau[i][j];
			...
			else if (plateau[i][j] == plateau[i+1][j]) //on teste bien que la tuile ne soit pas vide et que les deux adjacentes soient le résultat de 2^n pour un certain n
			{
				...
				plateau[i+1][j] = 0; //on vide la tuile du gauche car elle a fusioné avec la tuile initiale
				score += plateau[i][j];
			}
		}
	pair<Plateau, int> ok = { plateau, score };
		...
		for (int j = 3; j >= 0; j--)
		{
			if (plateau[j][i] == 0)
			{
				for (int count = j - 1; count >= 0; count--) // boucle permettant dans un premier point de tout décaller à droite 
					if (plateau[count][i])
					{
						plateau[j][i] = plateau[count][i]; //prend la valeur de la case d'à côté (droite)
	...
	for (int i = 0; i < 4; i++)
		for (int j = 3; j >= 0; j--)
		{
			if (plateau[j][i] == 0) // cas dans lequel la case est vide 
			{
				for (int count = j - 1; count > 0; count--) // ne prend pas en compte la ligne de droite et commence directement a la deuxieme case 
				{
					...
					if (plateau[count][i] == plateau[count - 1][i])
					{
						...
						plateau[count - 1][i] = 0; // remet a 0 la case de gauche qui vient d'être ajouter a la tuile initiale
						plateau[count][i] = 0;
						score += plateau[j][i];
			...
			else if (plateau[j][i] == plateau[j-1][i]) //on teste bien que la tuile ne soit pas vide et que les deux adjacentes soient le résultat de 2^n pour un certain n
			{
				...
				plateau[j - 1][i] = 0; //on vide la tuile dde droite car elle vient de fusionner avec la tuile initiale
				score += plateau[j][i];
			}
		}
	pair<Plateau, int> ok = { plateau, score };
...
pair<Plateau, int> deplacement(Plateau plateau) {
	...
	cin >> direction;
		while( stop == false){
			...
			switch (y) {                                  // attend de l'utilisateur d'appuyer sur une touche (ici définie préalablement
...
bool estGagnant(Plateau plateau) {
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 4; j++) {
			if (plateau[i][j] == 2048) {          // dans cette version classique du 2048, si on rencontre une tuille valant 2048 
...
void jeu() {
	...
	if (estTermine(plateau) == true) {
		cout << " Dommage, vous avez perdu:( " << " voulez-vous réessayer ? press m. Sinon press q pour quitter" << endl;
		char n;
		cin >> n;
		if (n == 'q') {
			exit(0);
